...
public class FlexboxLayoutManager extends RecyclerView.LayoutManager implements FlexContainer {
    ...
    private boolean updateAnchorFromChildren(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        // TODO: Reference child needs to be either a flex line (when FlexWrap is WRAP) or
        // a view (when FlexWrap is NOWRAP)
    ...
    private View findReferenceChild(int start, int end, int itemCount) {
        ...
        final int boundStart = mOrientationHelper.getStartAfterPadding();
        final int boundEnd = mOrientationHelper.getEndAfterPadding();
    ...
    /**
     * Fills the remaining space defined by the layoutState on how we should fill such as
     * the how many pixels should be filled (defined by {@link LayoutState#mAvailable}.
    ...
    private int fill(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        if (DEBUG) {
            Log.d(TAG, String.format("Start fill. childCount: %d, %s, %s", getChildCount(),
        ...
        if (DEBUG) {
            Log.d(TAG, String.format("End fill. childCount: %d, %s, %s", getChildCount(),
                    mAnchorInfo, mLayoutState));
        }
